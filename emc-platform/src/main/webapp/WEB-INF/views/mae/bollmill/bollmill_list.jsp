...
<head>
    ...
    <title>球磨机</title>
    <script src="${ctx}/script/mae/huak.mae.bollmill.list.js"></script>
</head>
<body class="gray-bg">
                    ...
                    <form id="bollmill-from">
                        ...
                        <div class="row btn-margin-bottom0">
                            <div class="col-lg-8  btn-group">
                                <c:if test="${sessionScope._auth['bollmillAdd']}">
                                    <button type="button" class="btn btn-sm btn-info J_menuItem" data-href="${ctx}/bollmill/add">
                                        <i class="fa fa-plus"></i>添加
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['bollmillUpdate']}">
                                    <button type="button" class="btn btn-sm btn-info" onclick="editBollmill()">
                                        <i class="fa fa-edit"></i>编辑
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['bollmillDelete']}">
                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteBollmills()">
                                        <i class="fa fa-trash-o"></i>删除
                                    </button>
                                </c:if>
                            </div>
                            <div class="col-lg-4 btn-text-align">
                                <button type="button" class="btn btn-sm btn-primary" onclick="getBollmillList()"> 搜索
                                </button>
                                <button type="reset" class="btn btn-sm btn-success"> 重置</button>
                                <c:if test="${sessionScope._auth['bollmillExport']}">
                                <button type="button" class="btn btn-sm btn-outline btn-info excel-export-btn" export-url="${ctx}/bollmill/export" >导出</button>
                                </c:if>
                            </div>
                        </div>
                    </form>
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        ...
        <td>
            <c:if test="${sessionScope._auth['bollmillDelete']}">
                <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="deleteBollmill('{$T.item.mae_id}')">
