...
<head>
    ...
    <title>送风机</title>
    <script src="${ctx}/script/mae/huak.mae.bfan.list.js"></script>
</head>
<body class="gray-bg">
                        ...
                        <div class="row btn-margin-bottom0">
                            <div class="col-lg-8  btn-group">
                                <c:if test="${sessionScope._auth['bfanAdd']}">
                                    <button type="button" class="btn btn-sm btn-info J_menuItem" data-href="${ctx}/bfan/add">
                                        <i class="fa fa-plus"></i>添加
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['bfanUpdate']}">
                                    <button type="button" class="btn btn-sm btn-info" onclick="editBfan()">
                                        <i class="fa fa-edit"></i>编辑
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['bfanDelete']}">
                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteBfans()">
                                        <i class="fa fa-trash-o"></i>删除
                                    </button>
                                </c:if>
                            </div>
                            <div class="col-lg-4 btn-text-align">
                                <button type="button" class="btn btn-sm btn-primary" onclick="getBfanList()"> 搜索
                                </button>
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        ...
        <td>
            <c:if test="${sessionScope._auth['bfanDelete']}">
                <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="deleteBfan('{$T.item.mae_id}')">
