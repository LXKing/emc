...
<head>
    ...
    <title>汽轮机</title>
    <script src="${ctx}/script/mae/huak.mae.turbine.list.js"></script>
</head>
<body class="gray-bg">
                    ...
                    <form id="turbine-from">
                        ...
                        <div class="row btn-margin-bottom0">
                            <div class="col-lg-8  btn-group">
                                <c:if test="${sessionScope._auth['turbineAdd']}">
                                    <button type="button" class="btn btn-sm btn-info J_menuItem" data-href="${ctx}/turbine/add">
                                        <i class="fa fa-plus"></i>添加
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['turbineUpdate']}">
                                    <button type="button" class="btn btn-sm btn-info" onclick="editTurbine()">
                                        <i class="fa fa-edit"></i>编辑
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['turbineDelete']}">
                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteTurbines()">
                                        <i class="fa fa-trash-o"></i>删除
                                    </button>
                                </c:if>
                            </div>
                            <div class="col-lg-4 btn-text-align">
                                <button type="button" class="btn btn-sm btn-primary" onclick="getTurbineList()"> 搜索
                                </button>
                                <button type="reset" class="btn btn-sm btn-success"> 重置</button>
                                <c:if test="${sessionScope._auth['turbineExport']}">
                                <button type="button" class="btn btn-sm btn-outline btn-info excel-export-btn" export-url="${ctx}/turbine/export" >导出</button>
                                </c:if>
                            </div>
                        </div>
                    </form>
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        ...
        <td>
            <c:if test="${sessionScope._auth['turbineDelete']}">
                <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="deleteTurbine('{$T.item.mae_id}')">
