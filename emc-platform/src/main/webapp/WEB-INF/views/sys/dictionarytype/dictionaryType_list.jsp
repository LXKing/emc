...
<head>
    ...
    <title>字典类型管理</title>
    <script src="${ctx}/script/sys/huak.sys.dictionarytype.list.js"></script>
</head>
<body class="gray-bg">
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form role="form" id="dictionaryType-form" >
                        <input type="hidden" name="_method" value="PATCH">
                        <input type="hidden" id="pageNo" name="pageNo" value="1">
                        <tr class="row col-md-15">
                            <label class="control-label col-md-1">类型编码</label>
                            <div class="form-group col-md-2">
                                <input  name="dicTypeCode" id="dicTypeCode" placeholder="类型编码"  class="form-control">
                            </div>
                            <label class="control-label col-md-1">类型名称</label>
                            <div class="form-group col-md-2">
                                <input name="dicTypeDes" id="dicTypeDes" placeholder="类型名称"  class="form-control">
                            </div>
                            <label class="control-label col-md-1">类型标识</label>
                            <div class="form-group col-md-2">
                                <input name="dicTypeName" id="dicTypeName" placeholder="类型标识"  class="form-control">
                            </div>
                        </tr>
                    ...
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tbody id="dictionaryType-tbody"></tbody>
                    </table>
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        <td>
            <input type="checkbox" class="i-checks" value="{$T.item.dicTypeId}" name="input[]">
        </td>
        <td>{$T.item.dicTypeCode}</td>
        <td>{$T.item.dicTypeDes}</td>
        <td>{$T.item.dicTypeName}</td>
        <td title="{$T.item.seq}">{formatText($T.item.seq,10)}</td>
        <td>
            <a class="updCom btn btn-info btn-xs btn-bitbucket" title="修改" onclick="editDictionary('{$T.item.dicTypeId}')">
                <i class="fa fa-file-text-o"></i>
            </a>
            <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="delDictionary('{$T.item.dicTypeId}')">
