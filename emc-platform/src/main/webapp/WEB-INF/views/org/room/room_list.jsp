...
<head>
    ...
    <title>房间管理</title>
    <script src="${ctx}/script/org/huak.org.room.list.js"></script>
</head>
<body class="gray-bg">
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                        <form role="form" id="room-search-form" >
                            <input type="hidden" name="_method" value="PATCH">
                            <input type="hidden" id="pageNo" name="pageNo" value="1">
                            <input type="hidden"  name="typeId" value="${typeId}">
                            <tr class="row col-md-15">
                                <label class="control-label col-md-1">房间名称</label>
                                <div class="form-group col-md-2">
                                    <input placeholder="房间名称" name="orgName"  class="form-control">
                                </div>
                                <label class="control-label col-md-1">所属单元</label>
                                <div class="form-group col-md-2">
                                    <div class="input-group colorpicker-demo3">
                                        <input  name="pOrgId" value=""  type="hidden"/>
                                        <input type="text" placeholder="所属单元" readonly="readonly" value="" class="form-control" />
                            ...
                            <div class="row btn-margin-bottom0">
                                <div class="col-lg-8  btn-group">
                                    <c:if test="${sessionScope._auth['bfanAdd']}">
                                        <button type="button" class="btn btn-sm btn-inf" onclick="addroom()">
                                            <i class="fa fa-plus"></i>添加
                                        </button>
                                    </c:if>
                                    <c:if test="${sessionScope._auth['bfanUpdate']}">
                                        <button type="button" class="btn btn-sm btn-info" onclick="editroom()">
                                            <i class="fa fa-edit"></i>编辑
                                        </button>
                                    </c:if>
                                </div>
                                <div class="col-lg-4 btn-text-align">
                                    <button type="button" class="btn btn-sm btn-primary" onclick="searchroom()"> 搜索</button>
                                    <button type="button" class="btn btn-sm btn-primary" onclick="resetroomSearch()"> 重置</button>
                    ...
                    <table class="table table-bordered table-striped table-hover" id="room-list-table">
                        <thead>
                        <tr>
                            <th width="7%">
...
<textarea id="room-datalist" style="display: none">
    ...
    <tr>
        ...
        <td >{$T.item.memo}</td>
        <td>
            <a class="btn btn-white btn-xs btn-bitbucket" title="修改" onclick="editroom('{$T.item.orgId}')">
                <i class="fa fa-file-text-o"></i>
            </a>
            <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="delroom('{$T.item.orgId}')">
