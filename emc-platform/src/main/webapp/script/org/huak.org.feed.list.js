        ...
        $.ajax({
            ...
            success: function (result) {
                $(".pagination").pagination({
                    ...
                    _callBack: getFeedList
                });
                // 附上模板
                $("#feed-tbody").setTemplateElement("tpl-list");
                // 给模板加载数据
                $("#feed-tbody").processTemplate(result.list);
                $('.i-checks').iCheck({
...
var delFeed = function (id){
    var ids;
    if(id == undefined){
        ids= getCheckValues("feedListTable");
        if (ids.length == 0) {
