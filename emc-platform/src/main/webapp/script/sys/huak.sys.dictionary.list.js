...
function getDictionaryList() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            // 附上模板
            $("#dictionary-tbody").setTemplateElement("tpl-list");
            // 给模板加载数据
            $("#dictionary-tbody").processTemplate(result.list);
            $('.i-checks').iCheck({
...
function addDictionary() {
    $('#layer-div').html("");
    $.get(ctx + '/dictionary/add', function (result) {
...
var editDictionary = function (id){
    ...
    $.get(ctx + '/dictionary/edit/' + ids, function (result) {
        $('#layer-div').html(result);
    });
    layer.open({
        area: ['600px', '500px'],
