...
<head>
    ...
    <title>热源管理</title>
    <script src="${ctx}/script/org/huak.org.feed.list.js"></script>
</head>
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form role="form" id="feed-form" >
                        <input type="hidden" name="_method" value="PATCH">
                        <input type="hidden" id="pageNo" name="pageNo" value="1">
                        <input type="hidden" name="typeId" value="${typeId}">
                        <tr class="row col-md-15">
                            <label class="control-label col-md-1">热源名称</label>
                            <div class="form-group col-md-2">
                                <input name="orgName" id="orgName" placeholder="热源名称"  class="form-control">
                            </div>
                            <label class="control-label col-md-1">热源性质</label>
                            <div class="form-group col-md-2">
                                    <select id="feedType" name="feedType" class="chosen-select form-control">
                                        ...
                                        <c:forEach items="${dicList['natureheat']}" var="types">
                                            <option value="${types.dicName}">${types.dicDes}</option>
                                        </c:forEach>
                                    </select>
                            </div>
                            <label class="control-label col-md-1">供热类型</label>
                            <div class="form-group col-md-2">
                        ...
                        <div class="row btn-margin-bottom0">
                            <div class="col-lg-8  btn-group">
                                <c:if test="${sessionScope._auth['bfanAdd']}">
                                    <button type="button" class="btn btn-sm btn-info J_menuItem"  data-href="${ctx}/feed/add">
                    ...
                    <table class="table table-bordered table-striped table-hover" id="feedListTable">
                        <thead>
                        <tr>
                            <th width="7%">
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        ...
        <td>
            ...
            此格式化方法只支持map集合--%>
            {formatId($T.item.feedType,"${dicDesMap['natureheat']}")}
        </td>
        <td>
            <%--
                参数1 ：当前后台获取字典标识
                参数2 ：获取外键表中map数据 格式：{key = 字典标识 ，value=显示字段}
            此格式化方法只支持map集合--%>
            {formatId($T.item.heatType,"${dicDesMap['supportheattype']}")}
        </td>
        <td>{$T.item.memo}</td>
        <td> {$T.item.addr} </td>
        <td>
            <a class="btn btn-info btn-xs btn-bitbucket J_menuItem"  data-href="${ctx}/feed/edit/{$T.item.orgId}" title="修改">
                <i class="fa fa-file-text-o"></i>
            </a>
            <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="delFeed('{$T.item.orgId}')">
