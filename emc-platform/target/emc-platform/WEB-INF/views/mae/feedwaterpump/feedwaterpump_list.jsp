...
<head>
    ...
    <title>补水泵</title>
    <script src="${ctx}/script/mae/huak.mae.feedwaterpump.list.js"></script>
</head>
<body class="gray-bg">
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form id="feedwaterpump-from">
                        ...
                        <div class="row btn-margin-bottom0">
                            <div class="col-lg-8  btn-group">
                                <c:if test="${sessionScope._auth['feedwaterpumpAdd']}">
                                    <button type="button" class="btn btn-sm btn-info J_menuItem" data-href="${ctx}/feedwaterpump/add">
                                        <i class="fa fa-plus"></i>添加
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['feedwaterpumpUpdate']}">
                                    <button type="button" class="btn btn-sm btn-info" onclick="editFeedwaterpump()">
                                        <i class="fa fa-edit"></i>编辑
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['feedwaterpumpDelete']}">
                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteFeedwaterpumps()">
                                        <i class="fa fa-trash-o"></i>删除
                                    </button>
                                </c:if>
                            </div>
                            <div class="col-lg-4 btn-text-align">
                                <button type="button" class="btn btn-sm btn-primary" onclick="getFeedwaterpumpList()"> 搜索
                                </button>
                                <button type="reset" class="btn btn-sm btn-success"> 重置</button>
                                <c:if test="${sessionScope._auth['feedwaterpumpExport']}">
                                <button type="button" class="btn btn-sm btn-outline btn-info excel-export-btn" export-url="${ctx}/feedwaterpump/export" >导出</button>
                                </c:if>
                            </div>
                        </div>
                    </form>
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tbody id="feedwaterpump-tbody"></tbody>
                    </table>
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        ...
        <td>
            <c:if test="${sessionScope._auth['feedwaterpumpDelete']}">
                <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="deleteFeedwaterpump('{$T.item.mae_id}')">
