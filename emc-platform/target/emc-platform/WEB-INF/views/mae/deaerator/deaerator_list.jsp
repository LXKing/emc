...
<head>
    ...
    <title>除氧器</title>
    <script src="${ctx}/script/mae/huak.mae.deaerator.list.js"></script>
</head>
<body class="gray-bg">
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form id="deaerator-from">
                        ...
                        <div class="row btn-margin-bottom0">
                            <div class="col-lg-8  btn-group">
                                <c:if test="${sessionScope._auth['deaeratorAdd']}">
                                    <button type="button" class="btn btn-sm btn-info J_menuItem" data-href="${ctx}/deaerator/add">
                                        <i class="fa fa-plus"></i>添加
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['deaeratorUpdate']}">
                                    <button type="button" class="btn btn-sm btn-info" onclick="editDeaerator()">
                                        <i class="fa fa-edit"></i>编辑
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['deaeratorDelete']}">
                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteDeaerators()">
                                        <i class="fa fa-trash-o"></i>删除
                                    </button>
                                </c:if>
                            </div>
                            <div class="col-lg-4 btn-text-align">
                                <button type="button" class="btn btn-sm btn-primary" onclick="getDeaeratorList()"> 搜索
                                </button>
                                <button type="reset" class="btn btn-sm btn-success"> 重置</button>
                                <c:if test="${sessionScope._auth['deaeratorExport']}">
                                <button type="button" class="btn btn-sm btn-outline btn-info excel-export-btn" export-url="${ctx}/deaerator/export" >导出</button>
                                </c:if>
                            </div>
                        </div>
                    </form>
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tbody id="deaerator-tbody"></tbody>
                    </table>
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        ...
        <td>
            <c:if test="${sessionScope._auth['deaeratorDelete']}">
                <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="deleteDeaerator('{$T.item.mae_id}')">
