...
<head>
    ...
    <title>热泵</title>
    <script src="${ctx}/script/mae/huak.mae.heatpump.list.js"></script>
</head>
<body class="gray-bg">
                    ...
                    <form id="heatpump-from">
                        ...
                        <div class="row btn-margin-bottom0">
                            <div class="col-lg-8  btn-group">
                                <c:if test="${sessionScope._auth['heatpumpAdd']}">
                                    <button type="button" class="btn btn-sm btn-info J_menuItem" data-href="${ctx}/heatpump/add">
                                        <i class="fa fa-plus"></i>添加
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['heatpumpUpdate']}">
                                    <button type="button" class="btn btn-sm btn-info" onclick="editHeatpump()">
                                        <i class="fa fa-edit"></i>编辑
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['heatpumpDelete']}">
                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteHeatpumps()">
                                        <i class="fa fa-trash-o"></i>删除
                                    </button>
                                </c:if>
                            </div>
                            <div class="col-lg-4 btn-text-align">
                                <button type="button" class="btn btn-sm btn-primary" onclick="getHeatpumpList()"> 搜索
                                </button>
                                <button type="reset" class="btn btn-sm btn-success"> 重置</button>
                                <c:if test="${sessionScope._auth['heatpumpExport']}">
                                <button type="button" class="btn btn-sm btn-outline btn-info excel-export-btn" export-url="${ctx}/heatpump/export" >导出</button>
                                </c:if>
                            </div>
                        </div>
                    </form>
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        ...
        <td>
            <c:if test="${sessionScope._auth['heatpumpDelete']}">
                <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="deleteHeatpump('{$T.item.mae_id}')">
