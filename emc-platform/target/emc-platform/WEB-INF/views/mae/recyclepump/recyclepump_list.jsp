...
<head>
    ...
    <title>循环泵</title>
    <script src="${ctx}/script/mae/huak.mae.recyclepump.list.js"></script>
</head>
<body class="gray-bg">
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <form id="recyclepump-from">
                        ...
                        <div class="row btn-margin-bottom0">
                            <div class="col-lg-8  btn-group">
                                <c:if test="${sessionScope._auth['recyclepumpAdd']}">
                                    <button type="button" class="btn btn-sm btn-info J_menuItem" data-href="${ctx}/recyclepump/add">
                                        <i class="fa fa-plus"></i>添加
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['recyclepumpUpdate']}">
                                    <button type="button" class="btn btn-sm btn-info" onclick="editRecyclepump()">
                                        <i class="fa fa-edit"></i>编辑
                                    </button>
                                </c:if>
                                <c:if test="${sessionScope._auth['recyclepumpDelete']}">
                                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteRecyclepumps()">
                                        <i class="fa fa-trash-o"></i>删除
                                    </button>
                                </c:if>
                            </div>
                            <div class="col-lg-4 btn-text-align">
                                <button type="button" class="btn btn-sm btn-primary" onclick="getRecyclepumpList()"> 搜索
                                </button>
                                <button type="reset" class="btn btn-sm btn-success"> 重置</button>
                                <c:if test="${sessionScope._auth['recyclepumpExport']}">
                                <button type="button" class="btn btn-sm btn-outline btn-info excel-export-btn" export-url="${ctx}/recyclepump/export" >导出</button>
                                </c:if>
                            </div>
                        </div>
                    </form>
                    <table class="table table-bordered table-striped table-hover">
                        ...
                        <tbody id="recyclepump-tbody"></tbody>
                    </table>
...
<textarea id="tpl-list" style="display: none">
    ...
    <tr>
        ...
        <td>
            <c:if test="${sessionScope._auth['recyclepumpDelete']}">
                <a class="btn btn-danger btn-xs btn-bitbucket" title="删除" onclick="deleteRecyclepump('{$T.item.mae_id}')">
