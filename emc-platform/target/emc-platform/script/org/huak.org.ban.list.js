...
function init() {
    ...
    function getBanList() {
        ...
        $.ajax({
            ...
            dataType: 'json',
            data: $("#ban-search-form").serialize(),
            success: function (result) {
                $(".pagination").pagination({
                    pageNo: result.list.page.curPage,
                    rowTotal: result.list.page.totalRow,
                    _callBack: getBanList
                });
                // 附上模板
                $("#ban-tbody").setTemplateElement("ban-list");
                // 给模板加载数据
                $("#ban-tbody").processTemplate(result.list);
                $('.i-checks').iCheck({
...
/*重置查询条件*/
var resetBanSearch = function(){
    $("#ban-search-form").find("div").find("input").val("");
    $("#searchManageTypeId").chosen("destroy").val("").chosen();
}

var delBan = function (id){
    var ids;
    if(id == undefined){
        ids= getCheckValues("banListTable");
        if (ids.length == 0) {
